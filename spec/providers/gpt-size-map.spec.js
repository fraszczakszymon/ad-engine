...
describe('gpt-size-map', () => {
	it('provides information about emptiness', () => {
		...
		expect(new GptSizeMap(sizes).isEmpty())
			.to.be.false;
		expect(new GptSizeMap([]).isEmpty())
			.to.be.true;
	});
	...
	it('adds new viewport size mappings to the collection', () => {
		...
		expect(sizeMap.sizeMap)
			.to.deep.equal([{ viewportSize: [728, 0], sizes: [[728, 90]] }]);
	});
